...
def _register_user(testapp, **kwargs):
    return testapp.post_json(
        url_for("user.register_user"), {"user": {"username": "mo", "email": "mo@mo.mo", "password": "momo"}}, **kwargs
    )
...
class TestAuthenticate:
    ...
    def test_user_login(self, testapp):
        ...
        resp = testapp.post_json(url_for("user.login_user"), {"user": {"email": "mo@mo.mo", "password": "momo"}})
    ...
    def test_get_user(self, testapp):
        ...
        resp = testapp.get(url_for("user.get_user"), headers={"Authorization": "Token {}".format(token)})
        assert resp.json["user"]["email"] == "mo@mo.mo"
